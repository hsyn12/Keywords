...
async function getStoredKeywordCounts() {
  try {
    ...
    return typeof result[STORAGE_KEY] === "object" &&
      result[STORAGE_KEY] !== null
      ? result[STORAGE_KEY]
      : {};
  } catch (error) {
    console.error(
      "[BG] Error getting keyword counts from session storage:",
      error
    );
...
async function saveAllCountsToStorage(allCounts) {
  try {
    ...
    console.error(
      "[BG] Error saving keyword counts to session storage:",
      error
    );
    return false;
  }
...
chrome.runtime.onInstalled.addListener(() => {
  ...
  chrome.storage.local.get(
    ["keywords", "highlightColor", "isActive"],
    (result) => {
      if (!result.keywords) {
  ...
  chrome.storage.session.remove(STORAGE_KEY, () => {
    if (chrome.runtime.lastError) {
      console.error(
        "[BG] Error clearing session storage on install:",
...
function getState(sendResponse) {
  ...
  chrome.storage.local.get(
    ...
    (result) => {
      ...
      response.isActive =
        typeof result.isActive !== "undefined" ? result.isActive : true;
    ...
    (async () => {
      try {
        ...
        if (typeof message.counts === "object" && message.counts !== null) {
          ...
          console.warn(
            "[BG] Received invalid counts data type:",
            typeof message.counts
          );
          sendResponse({
            success: false,
            error: "Invalid counts data received",
          });
        }
      } catch (error) {
  ...
  // New handler for popup to get counts for the active tab
    ...
    (async () => {
      try {
        ...
        console.error(
          `[BG] Error getting keyword counts for tab ${message.tabId}:`,
          error
        );
        sendResponse({ success: false, error: error.message });
      }
...
async function updateBadge(tabId) {
  ...
  try {
    ...
    if (
      ...
      console.error(
        `[BG] Error setting badge in updateBadge for tab ${tabId}:`,
        error
      );
    }
  }
  ...
  if (changeInfo.status === "loading") {
    ...
    try {
      ...
      if (
        ...
        console.error(
          `[BG] Error resetting badge on loading for tab ${tabId}:`,
          error
        );
      }
    }
        ...
        chrome.tabs.sendMessage(
          ...
          (response) => {
            ...
            if (
              ...
              console.error(
                `[BG] Error sending REHIGHLIGHT_TAB to tab ${tabId}:`,
                lastError.message
              );
            } else if (response && response.success === false) {
              console.warn(
                `[BG] Content script reported failure on rehighlight for tab ${tabId}:`,
                response.error
              );
            }
